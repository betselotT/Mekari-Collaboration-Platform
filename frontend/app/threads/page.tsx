...
import { ArrowRight, ArrowUp, Clock3, LogIn, MessageCircle, Search, Sparkles } from "lucide-react";
...
export default function PublicThreadsPage() {
  ...
  useEffect(() => {
    apiClient
      .get<{ threads: PublicThread[] }>("/api/threads/public")
  ...
  const totalReplies = threads.reduce((sum, thread) => sum + (thread.repliesCount || 0), 0);
  const totalUpvotes = threads.reduce((sum, thread) => sum + (thread.upvoteCount || 0), 0);
    ...
    <main className="min-h-screen bg-neutral-50 text-neutral-900 dark:bg-neutral-950 dark:text-white">
      <header className="border-b border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold">Mekari</span>
          </Link>
      ...
      <section className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-8 grid gap-6 lg:grid-cols-[1fr_320px]">
          <div>
            <Badge variant="info">Public threads</Badge>
            <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">
              Browse discussion threads
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-600 dark:text-neutral-400">
              Explore community questions, answers, and solved discussions before joining the conversation.
            </p>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      <Badge variant="info">{thread.subject || "General"}</Badge>
