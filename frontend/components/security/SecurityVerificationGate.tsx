...
export function SecurityVerificationGate({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        setError(
          err.response?.data?.error?.message ||
            ...
            t("Security verification failed. Please refresh the page and try again.")
        );
      }
    },
  ...
  const handleCaptchaError = useCallback(() => {
    setError(t("Security verification failed. Please refresh the page and try again."));
  }, [t]);
    ...
    <main className="flex min-h-screen items-center justify-center bg-white px-6 text-neutral-950 dark:bg-neutral-950 dark:text-white">
      <section className="w-full max-w-lg">
        ...
        <div className="border-y border-neutral-200 py-10 dark:border-neutral-800">
          ...
          <h1 className="text-2xl font-semibold">{t("Performing security verification")}</h1>
          <p className="mt-4 leading-7 text-neutral-600 dark:text-neutral-300">
            {t("This website uses a security service to protect against malicious bots. This page is displayed while the website verifies you are not a bot.")}
          </p>
          ...
          {state === "challenge" && (
            <div className="mt-8">
              <Captcha
                ...
                onExpired={() => setError(t("The verification expired. Please complete it again."))}
        ...
        <p className="mt-6 text-sm text-neutral-500 dark:text-neutral-400">
          {t("Mekari needs to review the security of your connection before proceeding.")}
        </p>
      </section>
    </main>
  );
