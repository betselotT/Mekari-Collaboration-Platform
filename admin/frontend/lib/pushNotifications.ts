...
export async function registerAdminPushNotifications() {
  ...
  if (!hasFirebaseConfig()) {
    return { ok: false, reason: "Firebase web push is not configured for the admin app." };
  }
  ...
  try {
    ...
    if (!token) return { ok: false, reason: "Firebase did not return a push token." };
    await adminFetch("/api/admin/push-token", {
      method: "POST",
      body: JSON.stringify({ token }),
    });
    window.localStorage.setItem("mekari_admin_fcm_token", token);
